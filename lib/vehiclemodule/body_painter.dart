...
class BodyPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    VehiclePartsInteractionBloc interactionBloc =
        context.read<VehiclePartsInteractionBloc>();
    ...
    if (contextSize.width > 1050) {
      ...
      //position
      double translateX =
          (contextSize.width - contextSize.width * 1.2 * xScale) / 2;
      double translateY =
          (contextSize.height * 0.96 - contextSize.height * 1 * yScale) / 2;
    ...
    // For mobile screens
      ...
      double translateY =
          (contextSize.height * 0.96 - contextSize.height * yScale) / 2;
    ...
    for (var muscle in generalParts!) {
      ...
      if (context.read<VehiclePartsInteractionBloc>().state.selectedBodyPart ==
          muscle.name) {
          ...
          if (!muscle.name.startsWith('text')) {
            context.read<VehiclePartsInteractionBloc>().add(
                ModifyVehicleInteractionStatus(
                    selectedBodyPart: muscle.name, isTapped: true));
          }
        },
      );
      if (displayAcceptedStatus &&
          interactionBloc.state.mapMedia.containsKey(muscle.name)) {
        ...
        } else if (interactionBloc.state.mapMedia[muscle.name]!.isAccepted ==
            false) {
          String rejectedPart = "";
