...
class VehiclePartsInteractionBloc2 extends Bloc<
    VehiclePartsInteractionBlocEvent2, VehiclePartsInteractionBlocState2> {
  NavigatorService? navigator;
  VehiclePartsInteractionBloc2({Repository? repo, this.navigator})
      : _repo = repo!,
        super(VehiclePartsInteractionBlocState2(
            mapMedia: {}, status: VehiclePartsInteractionStatus.initial)) {
    on<VehiclePartsInteractionBlocEvent2>((event, emit) {});
    on<AddCommentsEvent>(_onAddComments
        as EventHandler<AddCommentsEvent, VehiclePartsInteractionBlocState2>);
    on<AddImageEvent>(_onAddImage
        as EventHandler<AddImageEvent, VehiclePartsInteractionBlocState2>);
    on<RemoveImageEvent>(_onRemoveImage
        as EventHandler<RemoveImageEvent, VehiclePartsInteractionBlocState2>);
    on<SubmitBodyPartVehicleMediaEvent>(_onSubmitBodyPartVehicleMedia
        as EventHandler<SubmitBodyPartVehicleMediaEvent,
            VehiclePartsInteractionBlocState2>);
    on<FetchVehicleMediaEvent>(_onFetchVehicleMedia as EventHandler<
        FetchVehicleMediaEvent, VehiclePartsInteractionBlocState2>);
    on<ModifyAcceptedEvent>(_onModifyAcceptedStatus as EventHandler<
        ModifyAcceptedEvent, VehiclePartsInteractionBlocState2>);
    on<SubmitQualityCheckStatusEvent>(_onSubmitQualityCheckStatus
        as EventHandler<SubmitQualityCheckStatusEvent,
            VehiclePartsInteractionBlocState2>);
    on<ModifyVehicleExaminationPageIndex>(_onModifyVehicleExaminationPageIndex
        as EventHandler<ModifyVehicleExaminationPageIndex,
            VehiclePartsInteractionBlocState2>);
    on<AddHotspotEvent>(_onAddHotspot
        as EventHandler<AddHotspotEvent, VehiclePartsInteractionBlocState2>);
    on<RemoveHotspotEvent>(_onRemoveHotspot
        as EventHandler<RemoveHotspotEvent, VehiclePartsInteractionBlocState2>);
    ...
    on<ModifyVehicleInteractionStatus>(_onModifyVehicleInteractionStatus
        as EventHandler<ModifyVehicleInteractionStatus,
            VehiclePartsInteractionBlocState2>);
  ...
  void _onAddHotspot(
      AddHotspotEvent event, Emitter<VehiclePartsInteractionBlocState2> emit) {
    state.mapMedia.putIfAbsent(
      event.name,
      () {
        return VehiclePartMedia2(
            name: event.name,
            normalPosition: event.normal,
            dataPosition: event.position,
            isUploaded: false,
            images: [],
            comments: "");
      },
  ...
  void _onRemoveHotspot(RemoveHotspotEvent event,
      Emitter<VehiclePartsInteractionBlocState2> emit) {
    state.mapMedia.removeWhere((e, v) => e == event.name);
    emit(state.copyWith(mapMedia: state.mapMedia, status: state.status));
  }

  void _onAddComments(
      AddCommentsEvent event, Emitter<VehiclePartsInteractionBlocState2> emit) {
    if (state.mapMedia.containsKey(event.name)) {
      ...
      state.mapMedia.putIfAbsent(
        ...
        () {
          return VehiclePartMedia2(
              name: event.name, isUploaded: false, images: []);
        },
      );
    }
  ...
  void _onAddImage(
      AddImageEvent event, Emitter<VehiclePartsInteractionBlocState2> emit) {
    if (state.mapMedia.containsKey(event.name)) {
  ...
  void _onRemoveImage(
      RemoveImageEvent event, Emitter<VehiclePartsInteractionBlocState2> emit) {
  ...
  void _onSubmitBodyPartVehicleMedia(SubmitBodyPartVehicleMediaEvent event,
      Emitter<VehiclePartsInteractionBlocState2> emit) async {
    emit(state.copyWith(
        mapMedia: state.mapMedia,
        status: VehiclePartsInteractionStatus.loading));
    ...
    if (state.mapMedia[event.bodyPartName]!.images != null &&
      ...
      for (int i = 0;
          i < state.mapMedia[event.bodyPartName]!.images!.length;
          i++) {
    ...
    await _repo
        .addVehiclePartMedia(
            bodyPartData: partJson,
            id: event.serviceBookingNo,
            name: event.bodyPartName)
        .then((onValue) {
      state.mapMedia[event.bodyPartName]!.isUploaded = true;
      emit(state.copyWith(
          mapMedia: state.mapMedia,
          status: VehiclePartsInteractionStatus.success));
      emit(state.copyWith(
          mapMedia: state.mapMedia,
      ...
      (error, stackTrace) {
        emit(state.copyWith(
            mapMedia: state.mapMedia,
            status: VehiclePartsInteractionStatus.failure));
        emit(state.copyWith(
            mapMedia: state.mapMedia,
            status: VehiclePartsInteractionStatus.initial));
      },
  ...
  void _onFetchVehicleMedia(FetchVehicleMediaEvent event,
      Emitter<VehiclePartsInteractionBlocState2> emit) async {
    ...
    for (var entry in imageMedia.entries) {
      List<XFile> images = [];
      if (entry.value != {} &&
          entry.value["images"] != null &&
          entry.value["images"] != []) {
        for (int i = 0; i < entry.value["images"].length; i++) {
          Directory tempDir =
              Directory('${(await getTemporaryDirectory()).path}/${entry.key}');
          ...
          await imageFile
              .writeAsBytes(Base64Codec().decode(entry.value["images"][i]));
  ...
  void _onModifyAcceptedStatus(ModifyAcceptedEvent event,
      Emitter<VehiclePartsInteractionBlocState2> emit) {
    state.mapMedia[event.bodyPartName]!.isAccepted = event.isAccepted;
    emit(state.copyWith(mapMedia: state.mapMedia, status: state.status));
  }

  void _onSubmitQualityCheckStatus(SubmitQualityCheckStatusEvent event,
      Emitter<VehiclePartsInteractionBlocState2> emit) async {
    ...
    await _repo.addQualityStatus(qualityCheckJson: {
      "id": event.jobCardNo,
      "data": qualityCheckJson
    }).then((onValue) {
      emit(state.copyWith(
          status: VehiclePartsInteractionStatus.success,
          mediaJsonStatus: MediaJsonStatus.initial));
  ...
  void _onModifyVehicleExaminationPageIndex(
      ModifyVehicleExaminationPageIndex event,
      Emitter<VehiclePartsInteractionBlocState2> emit) {
    emit(state.copyWith(
        mapMedia: state.mapMedia,
        status: state.status,
        vehicleExaminationPageIndex: event.index));
  }

  void _onModifyVehicleInteractionStatus(ModifyVehicleInteractionStatus event,
      Emitter<VehiclePartsInteractionBlocState2> emit) {
    emit(state.copyWith(
        selectedBodyPart: event.selectedBodyPart, isTapped: event.isTapped));
  }

  void _onModifyRenamingStatus(ModifyRenamingStatus event,
      Emitter<VehiclePartsInteractionBlocState2> emit) {
    emit(state.copyWith(
        renamingStatus: event.renameStatus, renamedValue: event.renamedValue));
  }
}
